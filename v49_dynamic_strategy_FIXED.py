...
class V49DynamicStrategy:
    ...
    def check_buy_signal(self, symbol, current_date):
        ...
        # Extract scalar values directly using loc with column names
...
def compare_ytd_2025(initial_capital=20000, leverage_levels=[2.0, 2.25, 2.5, 2.75, 3.0, 3.25, 3.5]):
    """
    Run comparison for YTD 2025 only (Jan 1 - Oct 18, 2025)
    This tests the period where you achieved 190%+ CAGR
    """
    print("\n" + "╔" + "="*86 + "╗")
    print("║" + " "*86 + "║")
    print("║" + " "*22 + "V49 YTD 2025 COMPARISON" + " "*41 + "║")
    print("║" + " "*17 + "(January 1, 2025 - October 18, 2025)" + " "*30 + "║")
    print("║" + " "*86 + "║")
    print("╚" + "="*86 + "╝\n")
    
    # YTD 2025 date range
    start_date = '2024-12-01'  # Load a bit earlier for indicator calculation
    end_date = '2025-10-18'     # Current date
    ...
    for leverage in leverage_levels:
        for use_dynamic in [False, True]:
            mode = "Dynamic" if use_dynamic else "Static"
            print(f"\n{'='*80}")
            print(f"Testing: {mode} with {leverage}x Leverage (YTD 2025)")
            print(f"{'='*80}")
            ...
            # Load data including buffer for indicators
            ...
            trades_df = pd.DataFrame(strategy.trades)
            if len(trades_df) > 0:
                # Filter trades to 2025 only
                trades_df = trades_df[trades_df['entry_date'] >= '2025-01-01']
                
                if len(trades_df) > 0:
                    start_date_actual = trades_df['entry_date'].min()
                    end_date_actual = trades_df['exit_date'].max()
                    days = (end_date_actual - start_date_actual).days
                    years = days / 365.25
                    
                    # Annualized return for YTD
                    if years > 0:
                        cagr = (((final_equity / initial_capital) ** (1 / years)) - 1) * 100
                    else:
                        cagr = total_return
            ...
            results.append({
                'Mode': mode,
                'Leverage': f"{leverage}x",
                'YTD Return': f"{total_return:.1f}%",
                'Annualized': f"{cagr:.1f}%",
                'MaxDD': f"{strategy.max_drawdown*100:.1f}%",
                'Final': f"${final_equity:,.0f}",
                'Trades': len(trades_df)
            })
    ...
    # Print comparison table
    print("\n" + "="*110)
    print("YTD 2025 COMPARISON (Jan 1 - Oct 18, 2025)")
    print("="*110)
    ...
    print("="*110)
    print("\n💡 KEY INSIGHTS:")
    print("  • Shows performance in YOUR strong 2025 period")
    print("  • YTD Return = Actual return for ~10 months")
    print("  • Annualized = What this pace would mean over 12 months")
    print("  • Use this to see what leverage level hit your 190%+ target")
    print("="*110)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--compare':
            # Run full historical comparison
            compare_static_vs_dynamic(
                initial_capital=20000,
                leverage_levels=[2.0, 2.25, 2.5, 2.75, 3.0]
            )
        elif sys.argv[1] == '--ytd2025':
            # Run YTD 2025 comparison
            compare_ytd_2025(
                initial_capital=20000,
                leverage_levels=[2.0, 2.25, 2.5, 2.75, 3.0, 3.25, 3.5]
            )
        else:
            print("Usage:")
            print("  python v49_dynamic_strategy.py              # Single backtest")
            print("  python v49_dynamic_strategy.py --compare    # Full comparison")
            print("  python v49_dynamic_strategy.py --ytd2025    # YTD 2025 comparison")
    else:
        ...
        print("\n✅ Complete!")
        print("\n💡 Available commands:")
        print("   python v49_dynamic_strategy.py --compare    # Full historical comparison")
        print("   python v49_dynamic_strategy.py --ytd2025    # YTD 2025 comparison")
