...
"""
strategy_v48_fixed.py

Drop-in version with 1-D safe indicators to prevent:
ValueError: Data must be 1-dimensional, got ndarray of shape (N, 1)

Key fixes:
- Robust rsi() and macd() accept anything array-like and coerce to a flat float Series
- build_features() normalizes df["Close"] to a 1-D Series
- Extra guards where pandas might return a DataFrame instead of a Series
"""
...
def _to_series_1d(x, index=None) -> pd.Series:
    """Coerce any array-like / Series / DataFrame into a flat float Series with the given index if possible."""
    if isinstance(x, pd.DataFrame):
        if x.shape[1] == 1:
            x = x.iloc[:, 0]
        else:
            # If multiple columns, take the first by convention
            x = x.iloc[:, 0]
...
class MomentumAlignmentStrategy:
    ...
    def build_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["Close"] = _to_series_1d(close_col, getattr(close_col, "index", None)).astype(float)
        ...
        out["entry_ok"] = (
            out["trend_up"] & out["rsi_bull"] & out["macd_bull"] & out["adx_trendy"] & (out["sig_score"] > 0.15)
        )
...
class Backtester:
    ...
    def run(self):
        ...
        for i, (ts, row) in enumerate(self.df.iterrows()):
            ...
            if pos == 0.0 and can_cooldown and can_budget and bool(row["entry_ok"]):
                ...
                if np.random.rand() < p:
                    size_dollars = self.cfg.initial_equity * 0.0  # base
                    size_dollars = max(size_dollars, equity * min(0.25, self.risk.max_pos_risk_per_trade * 15))
                    size_dollars = min(size_dollars, self.strat.position_size(equity, row["daily_vol"]) * 10)
    ...
    def _stats(self) -> Dict[str, Any]:
        ...
        if not self.trades_df.empty:
            ...
            trades_per_week = 0.0
        return {
            "start_equity": self.cfg.initial_equity,
